...
SELECT DISTINCT s.id, s.name FROM student AS s JOIN payment AS p ON s.id = p.student_id WHERE (SELECT COUNT(*) FROM payment WHERE payment_date >= '2019-01-01' AND payment_date < '2020-01-01') > 2;